...
async function seed() {
  ...
  // 7. Seed Attendance (Current date)
  process.stdout.write('Seeding Attendance...\n');
  ...
  await recordRepo.save(class0Students.map(s => recordRepo.create({
  ...
  await targetRepo.save(class0Students.map(s => targetRepo.create({
  ...
  await assessmentRepo.save(class0Students.map((s, idx) => assessmentRepo.create({
    studentId: s.id,
