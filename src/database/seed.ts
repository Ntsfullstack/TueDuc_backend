...
async function seed() {
  ...
  const parentsData = [];
  for(let i=1; i<=20; i++) {
    parentsData.push({ email: `parent${i}@gmail.com`, password: hashedPassword, name: `Phụ Huynh ${i}`, role: Role.PARENT, phoneNumber: `09200000${i.toString().padStart(2, '0')}` });
  }
  const parents = await userRepo.save(parentsData.map(u => userRepo.create(u)));
  ...
  const homeworksData = [];
  for (let i = 1; i <= 30; i++) {
    homeworksData.push(homeworkRepo.create({
      title: `Bài tập đa thức ${i}`,
      description: `Làm bài tập trang ${40 + i} sách giáo khoa`,
      type: i % 2 === 0 ? HomeworkType.ESSAY : HomeworkType.QUIZ,
      dueAt: new Date(Date.now() + 86400000 * (i - 15)), // mix of past and future
      classId: classes[0].id,
      teacherId: teachers[0].id,
      targetScope: HomeworkTargetScope.CLASS,
    }));
  }
  const savedHomeworks = await homeworkRepo.save(homeworksData);

  const targetsData = [];
  const submissionsData = [];
  for (const hw of savedHomeworks) {
    for (const s of class0Students) {
      targetsData.push(targetRepo.create({
        homeworkId: hw.id,
        studentId: s.id,
      }));
      // 70% chance of submission
      if (Math.random() > 0.3) {
        submissionsData.push(submissionRepo.create({
          homeworkId: hw.id,
          studentId: s.id,
          parentId: s.parentId,
  ...
  await targetRepo.save(targetsData);
  await submissionRepo.save(submissionsData);
