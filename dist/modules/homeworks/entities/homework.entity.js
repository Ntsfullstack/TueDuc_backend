"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Homework = exports.HomeworkTargetScope = exports.HomeworkType = void 0;
const typeorm_1 = require("typeorm");
const class_entity_1 = require("../../classes/entities/class.entity");
const user_entity_1 = require("../../users/entities/user.entity");
const homework_target_entity_1 = require("./homework-target.entity");
var HomeworkType;
(function (HomeworkType) {
    HomeworkType["QUIZ"] = "quiz";
    HomeworkType["ESSAY"] = "essay";
})(HomeworkType || (exports.HomeworkType = HomeworkType = {}));
var HomeworkTargetScope;
(function (HomeworkTargetScope) {
    HomeworkTargetScope["CLASS"] = "class";
    HomeworkTargetScope["STUDENTS"] = "students";
})(HomeworkTargetScope || (exports.HomeworkTargetScope = HomeworkTargetScope = {}));
let Homework = class Homework {
    id;
    title;
    description;
    type;
    dueAt;
    class;
    classId;
    teacher;
    teacherId;
    targetScope;
    quiz;
    targets;
    createdAt;
    updatedAt;
};
exports.Homework = Homework;
__decorate([
    (0, typeorm_1.PrimaryGeneratedColumn)('uuid'),
    __metadata("design:type", String)
], Homework.prototype, "id", void 0);
__decorate([
    (0, typeorm_1.Column)(),
    __metadata("design:type", String)
], Homework.prototype, "title", void 0);
__decorate([
    (0, typeorm_1.Column)({ type: 'text', nullable: true }),
    __metadata("design:type", String)
], Homework.prototype, "description", void 0);
__decorate([
    (0, typeorm_1.Column)({ type: 'enum', enum: HomeworkType }),
    __metadata("design:type", String)
], Homework.prototype, "type", void 0);
__decorate([
    (0, typeorm_1.Column)({ name: 'due_at', type: 'timestamptz', nullable: true }),
    __metadata("design:type", Date)
], Homework.prototype, "dueAt", void 0);
__decorate([
    (0, typeorm_1.ManyToOne)(() => class_entity_1.Class),
    (0, typeorm_1.JoinColumn)({ name: 'class_id' }),
    __metadata("design:type", class_entity_1.Class)
], Homework.prototype, "class", void 0);
__decorate([
    (0, typeorm_1.Column)({ name: 'class_id' }),
    __metadata("design:type", String)
], Homework.prototype, "classId", void 0);
__decorate([
    (0, typeorm_1.ManyToOne)(() => user_entity_1.User),
    (0, typeorm_1.JoinColumn)({ name: 'teacher_id' }),
    __metadata("design:type", user_entity_1.User)
], Homework.prototype, "teacher", void 0);
__decorate([
    (0, typeorm_1.Column)({ name: 'teacher_id' }),
    __metadata("design:type", String)
], Homework.prototype, "teacherId", void 0);
__decorate([
    (0, typeorm_1.Column)({
        name: 'target_scope',
        type: 'enum',
        enum: HomeworkTargetScope,
        default: HomeworkTargetScope.CLASS,
    }),
    __metadata("design:type", String)
], Homework.prototype, "targetScope", void 0);
__decorate([
    (0, typeorm_1.Column)({ type: 'jsonb', nullable: true }),
    __metadata("design:type", Object)
], Homework.prototype, "quiz", void 0);
__decorate([
    (0, typeorm_1.OneToMany)(() => homework_target_entity_1.HomeworkTarget, (t) => t.homework),
    __metadata("design:type", Array)
], Homework.prototype, "targets", void 0);
__decorate([
    (0, typeorm_1.CreateDateColumn)({ name: 'created_at' }),
    __metadata("design:type", Date)
], Homework.prototype, "createdAt", void 0);
__decorate([
    (0, typeorm_1.UpdateDateColumn)({ name: 'updated_at' }),
    __metadata("design:type", Date)
], Homework.prototype, "updatedAt", void 0);
exports.Homework = Homework = __decorate([
    (0, typeorm_1.Entity)('homeworks')
], Homework);
//# sourceMappingURL=homework.entity.js.map